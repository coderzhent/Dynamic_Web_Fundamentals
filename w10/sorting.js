...
function searchList(l, q) {
  function searchCallBack(obj) {
    return (obj.name.toLowerCase().includes(q.toLowerCase()) 
          ...
          || obj.tags.find((el) => el.toLowerCase().includes(q.toLowerCase()))
        );
  }
  const filt = l.filter(searchCallBack);

  const sorted = filt.sort((a, b) => a.distance > b.distance);
  return sorted;
}
